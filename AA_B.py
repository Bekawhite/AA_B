    ...
    def create_referral(self, patient_data: Dict, user: Dict) -> Optional[Patient]:
        try:
            with self.db_service.get_session() as session:
                ...
                # Set referral time to current time
...
class AnalyticsService:
    ...
    def get_kpis(self) -> Dict[str, any]:
        with self.db_service.get_session() as session:
            ...
            # Start with zeros for all cost metrics
            total_fuel_cost = 0.0
            total_savings = 0.0
            total_distance = 0.0
            
            # Only calculate costs if there are completed trips
    ...
    def get_cost_analytics(self) -> Dict[str, any]:
        with self.db_service.get_session() as session:
            ...
            # Start with zeros for monthly data
            months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']
            monthly_costs = [0] * 6
            monthly_savings = [0] * 6
            
            # Only show data if there are completed trips
            if completed_trips:
...
class CostManagementUI:
        ...
        if cost_data['total_trip_costs'] > 0:
            ...
            with self.db_service.get_session() as session:
                ...
                if ambulances:
                    cost_distribution = []
                    for ambulance in ambulances:
                        if ambulance.total_fuel_cost > 0:  # Only show ambulances with costs
    ...
    def _display_fuel_management(self):
        ...
        if efficiency_data:
            ...
            st.info("No fuel efficiency data available yet")
    ...
    def _display_savings_analysis(self):
        ...
        if cost_data['total_trip_savings'] > 0:
            ...
            with self.db_service.get_session() as session:
                ...
                savings_data = []
                for ambulance in ambulances:
                    if ambulance.cost_savings > 0:  # Only show ambulances with savings
                        savings_data.append({
                            'Ambulance': ambulance.ambulance_id,
                            'Savings': ambulance.cost_savings,
...
class TrackingUI:
    ...
    def _display_comprehensive_map(self):
        ...
        for hospital_name, location in HOSPITAL_LOCATIONS.items():
            map_data.append({
                ...
                'name': hospital_name,
                'type': 'hospital',
                'color': [255, 0, 0],  # Red for hospitals
                'size': 100
            })
        ...
        with self.db_service.get_session() as session:
            ...
            for ambulance in ambulances:
                if ambulance.latitude and ambulance.longitude:
                    # Red for ambulance in motion, blue for available
                    color = [255, 0, 0] if ambulance.status == 'On Transfer' else [0, 0, 255]
                    map_data.append({
    ...
    def _display_tracking_info(self, patient, ambulance):
        if ambulance:
            ...
            if ambulance.latitude and ambulance.longitude:
                # Create map data with different colors
                map_data = pd.DataFrame({
                    ...
                    'name': ['Ambulance (Red)', 'Referring Hospital (Blue)', 'Receiving Hospital (Green)'],
            ...
            if ambulance.latitude and ambulance.longitude:
                ...
                map_data = pd.DataFrame({
                    ...
                    'lon': [ambulance.longitude, 
                           patient.referring_hospital_lng, 
                           patient.receiving_hospital_lng],
                    'name': ['Ambulance (Red)', 'Pickup (Blue)', 'Destination (Green)'],
                    'color': ['red', 'blue', 'green']
                })
